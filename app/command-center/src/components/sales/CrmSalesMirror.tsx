...
import {WeeklyWorkspace,financeViews,parseFinanceView,financeHref,type WeeklyFinanceView,type SalesClient} from '@proexteriors/sales-workspace';
...
export default function CrmSalesMirror({client,practice,homeHref='/',loginHref='/auth/login?returnTo=%2Fsales',logoutAction='/auth/logout',logoSrc='/pro-exteriors-logo.svg'}:{client:SalesClient;practice:boolean;homeHref?:string;loginHref?:string;logoutAction?:string;logoSrc?:string}){
 ...
 useEffect(()=>{setFinanceView(parseFinanceView(window.location.search));setRouteReady(true);},[]);
 ...
 return <div className={`wip-release${collapsed?' wip-rail-collapsed':''}`}>
  ...
  <aside className="wip-release-nav" aria-label="Customer journey">
   ...
   ].map((section,index)=><details key={section.title} open={index===3}><summary><span className="wip-nav-icon" aria-hidden="true">{['◎','◇','⚙','$','↗'][index]}</span><span>{section.title}</span></summary><p>{section.hint}</p><ul>{section.items.map(label=><li key={label}>{index===3?<a href={financeHref((Object.keys(financeViews) as WeeklyFinanceView[]).find(key=>financeViews[key].label===label)!,typeof window==='undefined'?'':window.location.search)} aria-current={financeViews[financeView].label===label?'page':undefined} onClick={e=>{if(pending.current||!ready)e.preventDefault();}}>{label}</a>:<span aria-disabled="true">{label}<small>Coming later</small></span>}</li>)}</ul></details>)}
  </aside>
  <div className="wip-release-layout">
   <header className="wip-release-header"><button type="button" className="wip-rail-toggle" aria-label="Toggle sidebar" aria-expanded={!collapsed} onClick={()=>setCollapsed(value=>!value)}>☰</button><div className="wip-release-title"><p>Job Finance Management</p><h1>{financeViews[financeView].label}</h1></div><div className="wip-session"><span>{practice?'Practice workspace':ready?'Staff session':'Verifying access'}</span>{role&&<small>{role==='admin'?'Administrator':role==='project_manager'?'Project manager':role==='manager'?'Sales manager':role==='sales_rep'?'Sales representative':'Staff workspace'}</small>}</div>
   ...
   <main id="wip-main" tabIndex={-1} className="wip-release-main">
    ...
    {error?<section className="wip-access-state" role="alert"><h1>Let’s reconnect your workspace</h1><p>{error}</p><button onClick={()=>setAttempt(n=>n+1)}>Check access again</button>{!practice&&<a href={loginHref}>Sign in</a>}</section>:!ready||!routeReady?<section className="wip-access-state" role="status"><h1>Opening your weekly review</h1><p>Checking your staff access and assigned contracts.</p></section>:<div className="sales-workspace"><WeeklyWorkspace initialView={financeView} onFinanceViewChange={financeChanged} client={client} practice={practice} embedded desktopBoard canManageRepAccess={role==='admin'} onDirty={value=>{dirty.current=value;}} onPending={value=>{pending.current=value;}}/></div>}
   </main>
  </div>
 </div>;
}
