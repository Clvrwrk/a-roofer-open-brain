...
## Environment / Deploy
Source = GitHub `Clvrwrk/a-roofer-open-brain`; **LIVE = `origin/main`** (Coolify auto-builds). Dev branch = origin/main = live (`72c70dd`). `git push origin HEAD:main` deploys; verify `curl https://cc.proexteriorsus.net/healthz`. **Local `main` is STALE — never use it.** Supabase `rnhmvcpsvtqjlffpsayu`; schemas mirrored thru **142**. Build: `cd app/command-center && npm run build`.
