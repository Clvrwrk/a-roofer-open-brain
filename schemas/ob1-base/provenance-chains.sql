...
-- internal MCP container, which authenticates via its access key and uses the
...
-- PostgREST and bypass the MCP access key entirely. Keep it
-- service_role only so the MCP container is the sole caller.
