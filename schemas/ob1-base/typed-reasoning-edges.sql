...
--      `decay_weight`) on the optional entity `edges` table, when that
--      table is present, so that edge relevance can decay over time.
--
-- Both parts are idempotent. The entity-extraction portion is guarded,
-- so it is safe to re-apply and is a no-op if the `edges` table has not
-- been installed in this brain.
--
-- PREREQUISITES
--   - `public.thoughts` table (from `docs/01-getting-started.md`)
--   - Optional: `schemas/entity-extraction/schema.sql` applied, when this
--     brain uses entity-to-entity graph edges in addition to thought edges.
--
...
--    is missing. Entity-extraction edges are optional in this roofer
--    template, so their absence should not block thought_edges.
-- ============================================================
...
--    entity-to-entity edges table when present. Idempotent: ADD COLUMN
--    IF NOT EXISTS means re-running is a no-op.
-- ============================================================
