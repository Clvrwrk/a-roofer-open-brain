-- 297 — an agreement that exists but reaches nothing. Detector + a corrected exposure gate.
-- Applied to prod 2026-08-20.
--
-- WHY: migration 296's `needs_ruling` keyed off `live_agreements = 0` — does the PAPERWORK
...
COMMENT ON VIEW public.v_agreement_unreachable IS
  ...
  'some lines. Cause is the branch-number TEXT join in v_office_vendor_branch (see mig 297 '
  'header).';

-- Corrected gate: auditability, not paperwork. Supersedes migration 296's predicate.
...
COMMENT ON VIEW public.v_office_vendor_gap_exposure IS
  'Coverage gaps with the ruling (mig 245) AND the dollars (mig 294), gated on whether the '
