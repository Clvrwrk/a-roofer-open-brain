-- 296 — put DOLLARS on the coverage gaps.
--
...
--   * IN THIS REPO: the file is `296-...`, renumbered SIXTEEN times as parallel sessions
--     claimed numbers on main while this work was in flight. Main is canonical, so this
--     branch yields every time. Intermediate numbers (250-253, 263-266, 267, 281-285,
--     286-288, 289-293, 292-296, 293-297, 294-298) no longer name any file in this set.
-- The sibling migrations of this set are 297, 298, 299 and 300, all applied to prod.
--
...
--  12th, 2026-09-12: main landed its own 285-288 (credit-memo guard, runtime status
--                    plumbing + feed freshness, materialised order/AccuLynx match)
--                                                                -> set moved 286-290 to 289-293
--  13th, 2026-09-15: main landed its own 289-291 (triage office race + reconcile scope,
--                    reopen June triage race lines, rekey ABC branch 326 Topeka)
--                                                                -> set moved 289-293 to 292-296
--  14th, 2026-09-15: main landed `292-alias-slug-keyed-abc-branches.sql` ~1 hour after the
--                    13th                                      -> set moved 292-296 to 293-297
--  15th, 2026-09-16: main landed `293-june-recycled-invoices-closeout.sql`
--                                                                -> set moved 293-297 to 294-298
--  16th, 2026-09-22: main landed `294-acculynx-link-ins-prefix-and-client-name.sql` and
--                    `295-weekly-export-pending-matches-app.sql`. The set left the
--                    contended range entirely this time and moved to 296-300, and moved
--                    OUT of the surface/docs PR into its own, so a sixteenth collision
--                    cannot drag the rest of that work with it.
--                                                                -> set moved 294-298 to 296-300
