-- 203 — invoice_audit_reset v2 (docs/82 R4). Applied to prod 2026-08-05.
-- The "Go back" reset, aligned with the v2 status model (docs/81):
--   * NEW guards: refuses vendor credit memos and the two v2 payment states
--     (paid_pending_verification / paid_verified) alongside exported/paid
--   * cancels DRAFT **and** APPROVED (un-sent) requested credit memos
--   * clears the human claim-line review acknowledgments (migration 202) so a
--     reset invoice starts its review from scratch
--   * syncs invoice_pipeline_status: audit_completed_at cleared; status returns to
--     invoice_audit_pending when current claim lines exist, else invoice_processed
--   * unchanged: append-only line re-pend, action log, sent communications untouched
CREATE OR REPLACE FUNCTION public.invoice_audit_reset(p_invoice_number text, p_actor_id text, p_actor_type text, p_actor_display text)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
  ...
  WHERE invoice_number = p_invoice_number AND reviewed_at IS NOT NULL;
