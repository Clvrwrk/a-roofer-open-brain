-- 256 — an agreement that exists but reaches nothing. Detector + a corrected exposure gate.
-- Applied to prod 2026-08-20.
--
-- WHY: migration 255's `needs_ruling` keyed off `live_agreements = 0` — does the PAPERWORK
...
COMMENT ON VIEW public.v_agreement_unreachable IS
  ...
  'nothing. Cause is the branch-number TEXT join in v_office_vendor_branch (see mig 256 '
  'header). Non-empty means money is auditing as no-price despite a signed book existing.';

-- Corrected gate: auditability, not paperwork. Supersedes migration 255's predicate.
...
COMMENT ON VIEW public.v_office_vendor_gap_exposure IS
  'Coverage gaps with the ruling (mig 245) AND the dollars (mig 253), gated on whether the '
