...
//   - advanceWatermark upserts with onConflict 'account_key,resource_type'
//     (LIVE column is resource_type — 02-01-SUMMARY critical deviation)
//   - readWatermark filters by BOTH account_key AND resource_type
...
Deno.test("advanceWatermark — upserts with onConflict 'account_key,resource_type'", async () => {
  ...
  assertEquals(
    options?.onConflict,
    "account_key,resource_type",
    "onConflict must be 'account_key,resource_type' (live column name)",
  );
});
...
Deno.test("advanceWatermark — passes the correct watermark row to upsert", async () => {
  ...
  assertEquals(passedRow.resource_type, "jobs");
...
Deno.test("readWatermark — filters by account_key", async () => {
  const wm: WatermarkRow = { account_key: "kansas_city", resource_type: "contacts", last_page_index: 0 };
...
Deno.test("readWatermark — filters by resource_type", async () => {
  const wm: WatermarkRow = { account_key: "kansas_city", resource_type: "contacts", last_page_index: 0 };
  const { sb, calls } = makeReadMock(wm);
  ...
  const eqCalls = calls.filter((c) => c.method === "eq");
  const resourceEq = eqCalls.find((c) => c.args[0] === "resource_type" && c.args[1] === "contacts");
  assertEquals(resourceEq !== undefined, true, "Must filter by resource_type (live column name)");
});

Deno.test("readWatermark — ('kansas_city','contacts') is independent of ('florida','contacts')", async () => {
  // kansas_city contacts watermark
  const kcWm: WatermarkRow = { account_key: "kansas_city", resource_type: "contacts", last_page_index: 100 };
  ...
  const flWm: WatermarkRow = { account_key: "florida", resource_type: "contacts", last_page_index: 25 };
