...
- **UOM & pricing (read before touching any price comparison):** compare invoice/order prices to agreements in ONE unit — ABC's pricing UOM (`priceQty.uom`, e.g. SQ). Canonical effective price = `abc_invoice_lines.price_per_uom` (`extendedPriceAmount ÷ priceQty.value`); align orders via `v_item_uom_map`; never compare on `quantity`/`uom`/`unit_price`/`effective_unit_price`/`pricePerUnitAmount`. Full contract: [`docs/46-uom-pricing-normalization.md`](docs/46-uom-pricing-normalization.md) and [`CONVENTIONS.md`](CONVENTIONS.md) §10c (migrations 119–122, 2026-06-19).
- **Price-agreement silos (read before touching any pricing join):** a negotiated price may only meet an invoice line if it passes all four gates — **vendor** (never join a branch by bare branch number; branch numbers collide across vendors, mig 208), **office** (resolved from the *invoice's* branch via `pricing_territory_office_id`; agreements are never shared between PE offices, mig 217), **time** (`effective_date <= invoice_date` plus **item-aware** version supersession — a newer version supersedes an older one only for the items it actually prices, so an item a shorter new list omits keeps its last known price; all agreements remain in effect until the vendor provides a new one, mig 277), and **UOM** (the audit refuses rather than converts). The fuzzy arm is always a fallback, and the final tie-break picks the LOWEST price — so simulate before adding or backdating a book into an office that already has one. A price list stays in effect until a **new list supersedes it**; `expiry_date` is documentary, recorded per agreement in `renewal_mode` (`evergreen` default | `expires`, mig 270). Read the audit through **`mv_invoice_audit_line`**, never `v_invoice_audit_line` — the view costs ~8.8s against an 8s `statement_timeout`, so every PostgREST read fails and the surfaces render empty (migs 272–273). Full contract: [`docs/105-price-agreement-silo-rules.md`](docs/105-price-agreement-silo-rules.md) and [`CONVENTIONS.md`](CONVENTIONS.md) §10b.
- **Vendor parity of the audit.** One eval per vendor; they differ **only** where the vendor's process differs. Legitimate differences (docs/81 d13 + open item 1): QXO has no agreements ever ⇒ every line valid as billed; SRS prices off the Level 4 sheet mapped to Richardson TX; PDF/OCR line-sum verification is ABC-only because no PDF source exists for SRS/QXO. Everything else — the four gates, UOM refusal, lowest-price tie-break, fuzzy fallback order, the No-Price `purchases_ytd >= 2` threshold, the credit-memo claim bar — is vendor-agnostic. Never special-case a vendor to make a number look right. Both arms are now at parity (mig 279): item-aware supersession runs on SRS/QXO too (previously absent — a cheaper *older* sheet would have won), and evergreen is one `COALESCE`-guarded predicate on all four arms. `renewal_mode` is `NOT NULL DEFAULT 'evergreen'` on both agreement tables. **Negative total = credit memo, never a payable** — route to `v_credit_memo_tbd` (cross-vendor, mig 280) for matching to its original invoice or a CM TBD line; derive from the amount, never write the flag onto the mirror (the nightly sync overwrites it). **The QB bank export is one file per vendor** — ABC, SRS and QXO keep separate QB bank registers, so a mixed-vendor export posts one vendor's invoices into another's register; `scripts/build-inv-processed-weekly.mjs` refuses to write a mixed file (supersedes docs/81 d2/d14, Chris 2026-08-25). When adding a vendor arm, port all four gates **plus** item-aware supersession. Full table: [`CONVENTIONS.md`](CONVENTIONS.md) §10b.
...
- **Design system (read before building or changing any surface, desktop or mobile):** the living design system is **`cc.proexteriorsus.net/design-system`** (source `app/command-center/src/pages/design-system/`, docs/112). Tokens are parsed from `global.css` at build time; every component renders live in both modes; every motion is catalogued; the UX decisions log (nesting, filters, tables, feedback) carries permanent rule ids. Start at `/design-system/agents` (build recipe + ship checklist), cite rule ids in the PR, and update the matching chapter in the same PR when a token, component, mode, motion or decision changes. Machine feed: `/design-system/tokens.json`.
...
- **The agent deploys; the gate is explain-then-ship.** The agent has full GitHub, Coolify, and Hetzner access and is expected to commit, push `origin main`, and poll the Coolify deploy to completion itself (corrected 2026-06-29; supersedes the prior "deploys are a human action" rule). Required discipline before any deploy: **state what is changing, its user-visible impact, and the rollback** — then carry out the push and confirm it landed. Do all prep first (converge branch → `main`, apply migrations, build + tests green), then push and watch `GET /healthz` `buildCommit` flip to the pushed SHA (≈30–90s, up to ~300s on a cold build). Tightly-scoped helper: `scripts/coolify-redeploy.sh`. See the **`/coolify`** skill.
...
**Canonical procedure: [`CONVENTIONS.md`](CONVENTIONS.md) §13** — shared across all harnesses. This section is the Claude Code copy; keep the two in sync (see step 6, Agent alignment).

When the user says **"handoff"**, **"wrapup"**, **"wrap up"**, **"end of session"**, **"tie off"**, or invokes **`/wrapup`** — or when context usage reaches ~50% — run this checklist in order and do not stop until the working tree is clean and converged. The goal: the next session starts on a clean, current `main` and immediately knows where work left off.
...
5. **Agent alignment.** Verify every harness instruction file carries the same instruction set. If a rule, hard rule, memory budget, or this wrap-up procedure changed, propagate it to **all** of: `CLAUDE.md` (this file), [`AGENTS.md`](AGENTS.md), [`CONVENTIONS.md`](CONVENTIONS.md) §13 (the source of truth), and `.cursor/rules/*.mdc` — plus any new harness file the team has added. Commit the alignment in the same wrap-up. Files may differ in framing but must never contradict each other or CONVENTIONS §13.
6. **Report.** One message: branch + last commit hash/msg, `tree clean ✓` (or what remains and why), what was accomplished, the exact next task, and any blocker needing the user. Then stop — do not start the next task.
