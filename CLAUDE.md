...
- **Price-agreement silos (read before touching any pricing join):** a negotiated price may only meet an invoice line if it passes all four gates — **vendor** (never join a branch by bare branch number; branch numbers collide across vendors, mig 208), **office** (resolved from the *invoice's* branch via `pricing_territory_office_id`; agreements are never shared between PE offices, mig 217), **time** (`effective_date <= invoice_date` plus version supersession), and **UOM** (the audit refuses rather than converts). The fuzzy arm is always a fallback, and the final tie-break picks the LOWEST price — so simulate before adding or backdating a book into an office that already has one. Full contract: [`docs/105-price-agreement-silo-rules.md`](docs/105-price-agreement-silo-rules.md) and [`CONVENTIONS.md`](CONVENTIONS.md) §10b.
