...
| F42 | **ABC branch 326 (Topeka) was not "unmapped" — it was mapped under a key the ingest could not resolve.** `vendor_branches` carried the Topeka row from the 2026-05-27 location import as `branch_number = 'topeka-KS-66618-1445'`, already assigned to Kansas City by isochrone containment (`pricing_status = covered`) — the drive-time rule worked. Since mig 243 an ABC invoice resolves its branch AT INGEST through `vendor_branch_alias` into `abc_invoices.vendor_branch_id`, and `v_invoice_pricing_office` joins on that FK; mig 243 seeded aliases only for numeric-keyed rows, so the slug-keyed Topeka row had none, 2014501859-001 ingested with a NULL FK, resolved to no office, and every line read No-Price. The map looks the branch up by the bare number, found no row, and showed "Kansas City · suggested / Not yet assigned"; its Save button would have stamped the slug row and left the FK NULL. Only 326 was affected (the one invoice since June with a NULL FK); 685 of 761 ABC rows still carry slug keys and 25 numeric rows have a same-city slug twin from mig 239. **mig 291 (Chris: "ship it")**: row re-keyed to `326`, invoice FK backfilled, aliases `326`/`0326` added, chain refreshed, and the three lines the Kansas City list prices over agreement reopened (05ABCPG20 $0.79, 06MHSAB $20.16, GRV20ML $10.70 fuzzy; $31.65) | SQL; `/api/invoice-audit/invoice?invoiceNumber=2014501859-001` |
| F43 | **mig 292 (Chris: "alias every slug-keyed branch row to its master number")**: 670 alias rows written for 589 of the 684 slug-keyed ABC rows, matched to the branch master by ZIP+4 (465), 5-digit ZIP + city (1) or unique city (123), both spellings per mig 243; 25 slug rows are twins of a numeric resolver (left as duplicates); 70 stay unaliased (Canadian branches, multi-branch US cities with no unique key). All 62 master branches bought from in 36 months resolve at ingest; 678 of 698 masters overall. **292b** fixed the mirror-image case found while verifying: four numeric stub rows owned the alias but carried no office while their slug twins held the isochrone assignment — 1278 Granbury and 305 Sherman → Richardson, 1306 Enid → Wichita, 185 Marietta → Atlanta; the assignment now lives on the stub (the ingest identity). Sherman's three Jan-2026 invoices had priced as No-Price through the stub; all three are paid, so nothing reopened | SQL |
| F44 | **mig 293 (Chris 2026-09-16: "the invoices we recycled from ABC month 06 can be marked processed with a note … never brought back into the invoice audit workflow")**: new `invoice_audit_closeout` register (7 June invoices, human-attributed reason); the 8 still-pending lines decided valid under Chris's name with the reason in the note; `invoice_payment_processed` 'paid' rows (source `closeout`) for the six invoices with no ledger row so the app reads them processed and they leave the To-Be-Paid set; pipeline status `invoice_processed`; `invoice_audit_reset()` now refuses a closed-out invoice (`invoice_closed_out`) before writing anything — probed live. Then 293b (Chris: "cancel the $20 draft CM"): the draft request on 2011009179-001 cancelled with the closeout reason and its disputed line decided valid. Left alone: 2011284893-001's 7/27 `paid_pending_verification` row (Lucinda's queue). The app now reads the register: `closedOut` on every invoice loader, register label and tree pill "Processed — closed", Go back hidden. Any future reopen script must check the closeout register first | SQL; `/api/invoice-audit/invoice` |
...
- 2026-09-15 (later) — mig 291: branch 326 re-keyed + aliased + FK backfilled; F42 rewritten with the mig-243 mechanism.
- 2026-09-15 (later) — mig 292/292b: slug-keyed ABC branches aliased to their master numbers; four numeric stubs given their office (F43).
- 2026-09-16 — mig 293: June recycled invoices closed out (F44); `invoice_audit_closeout` register + reset guard.
