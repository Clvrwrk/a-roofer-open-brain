...
**256-259** (already applied to prod, all additive) plus `docs/102`. It is 0 behind main and
kept merged with it. **It is not merged and not deployed.** Numbering note: parallel sessions
claimed 245-255 and both `docs/99` and `docs/100` while it was in flight, so it yielded five
times — its applied
...
- **`vendor_invoices` has two BEFORE triggers** now — branch resolution (243) and PO
  canonicalization (257). Anything writing to that table gets both.
