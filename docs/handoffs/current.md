...
**289-293** (already applied to prod, all additive) plus `docs/107`. It is 0 behind main and
...
3. Four branches (21, 39, 465, 684) are geocoded but marked `geocode_status = 'pending'`,
   against a `geom IS NOT NULL` ⇒ `'ok'` invariant that holds for 1,752 rows. Mig 290
...
4. `v_office_vendor_branch` and `v_office_vendor_inheritance` are readable by `anon` on the
   same default grants that mig 293 closed for the four coverage views. They predate this
...
3. **4 branches geocoded but `geocode_status = 'pending'`** (21, 39, 465, 684) against a
   `geom IS NOT NULL` ⇒ `'ok'` invariant holding for 1,752 rows. Mig 290 demoted two; 39
