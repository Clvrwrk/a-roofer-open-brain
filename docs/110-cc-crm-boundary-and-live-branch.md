...
**Date:** 2026-09-11 · **Asked by:** Chris · **Status:** Option A approved and **executed 17:07 UTC** (§4 steps 1, 3, 4, 6-archive done; step 2 not needed — `COMMAND_CENTER_PUBLIC_URL` falls back to the request origin; step 5 due 2026-09-12 — `docker stop cc-production-e4344d8`, operator action (docs/109 F29); the CRM-runbook contract in §3 still to be filed in `CRM_PWA`)
...
| Data | CRM: `crm`, `crm_private`, `crm_gateway`. CC: `public`, `runtime_*`, `qbo_registers`, `jt_mirror`, … | Additive only (hard rule 1). CC reads CRM data only through `crm_gateway` objects the CRM publishes for it. **Known exceptions the CRM already made** (PostgREST pre-request hook on `authenticator`, ACL rewrites in `public`/`storage`/`graphql_public`, policy + column grant + FKs + inserts on `public.properties`) are inventoried in [docs/111](111-crm-pwa-companion-repo.md) and must not grow without a joint review. |
